...
export default function Checkout() {
    ...
    if (data && !error) {
        return (
            <Modal open={userProgressCtx.progress === 'checkout'} onClose={handleCloseFinished}>
                <h2>Success!!</h2>
                <p>Your order was submitted Successfully..</p>
                <p>We ll get back to you with more details via email within the next few minutes.</p>
                <p className="modal-actions">
                    <Button onClick={handleCloseFinished}>Okay</Button>
                </p>
            </Modal>
        )
    }
    ...
    return <Modal open={userProgressCtx.progress === 'checkout'} onClose={handleCloseCheckout}>
        <form onSubmit={handleCheckout}>
            ...
            {error && <Error message={error} title="Failed to submit the request" />}
            <p className="modal-actions">
                {actions}
            </p>
        </form>
